...
class Routes(
  ...
  def this(errorHandler: play.api.http.HttpErrorHandler,
    ...
    // @LINE:4
    Application_0: controllers.Application
  ) = this(errorHandler, webknossos_versioned_Routes_0, com_scalableminds_webknossos_datastore_Routes_1, Application_0, "/")

  def withPrefix(addPrefix: String): Routes = {
    val prefix = play.api.routing.Router.concatPrefix(addPrefix, this.prefix)
    noTS.RoutesPrefix.setPrefix(prefix)
    new Routes(errorHandler, webknossos_versioned_Routes_0, com_scalableminds_webknossos_datastore_Routes_1, Application_0, prefix)
  }
